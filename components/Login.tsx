...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const renderLoginForm = () => (
    <form onSubmit={handleLogin} className="bg-white rounded-[2rem] p-8 border border-slate-100 shadow-sm space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-[10px] text-slate-400 uppercase tracking-widest font-bold">o</span>
        <div className="flex-1 h-px bg-slate-200"/>
      </div>
      <button type="button" onClick={handleGoogleLogin} disabled={loading}
        className="w-full py-3.5 border border-slate-200 bg-white text-slate-700 font-semibold rounded-xl hover:bg-slate-50 active:scale-[0.98] transition-all disabled:opacity-40 flex items-center justify-center gap-3 text-sm shadow-sm">
  ...
  const renderRegisterForm = () => (
    <form onSubmit={handleRegisterSearch} className="bg-white rounded-[2rem] p-8 border border-slate-100 shadow-sm space-y-5">
      ...
      <div className="flex items-center gap-3">
        <div className="flex-1 h-px bg-slate-200"/>
        <span className="text-[10px] text-slate-400 uppercase tracking-widest font-bold">o más rápido</span>
        <div className="flex-1 h-px bg-slate-200"/>
      </div>
      <button type="button" onClick={handleGoogleLogin} disabled={loading}
        className="w-full py-3.5 border border-slate-200 bg-white text-slate-700 font-semibold rounded-xl hover:bg-slate-50 active:scale-[0.98] transition-all disabled:opacity-40 flex items-center justify-center gap-3 text-sm shadow-sm">
