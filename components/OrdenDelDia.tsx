...
type Tab = 'viernes' | 'domingo' | 'mes' | 'equipo' | 'novedades';
...
interface ProgramaMes {
  ...
  liderCargo: string;
  temasMes: string[];
  viernes: ServicioViernes[];
  domingos: ServicioDomingo[];
  ayunos: Ayuno[];
  eventos: EventoMes[];
}
...
const PROGRA_FIELDS: Record<PrograSection, { key: string; label: string; placeholder: string }[]> = {
  viernes: [
    ...
    { key: 'oracion',    label: 'Oración / Ofrenda / Despedida', placeholder: 'Responsable' },
  ],
  domingos: [
...
const OrdenDelDia: React.FC<Props> = ({ role }) => {
  ...
  const [state, setState] = useState<OrdenState>(() => {
    try {
      ...
      if (saved) {
        ...
        return { ...DEFAULT_STATE, ...p, programaMes: p.programaMes ?? DEFAULT_STATE.programaMes };
      }
    } catch { /* ignore */ }
  ...
  // Week selector
  const [selectedViernesId, setSelectedViernesId] = useState<string | null>(null);
  const [selectedDomingoId, setSelectedDomingoId] = useState<string | null>(null);
  ...
  const setCurrentItems = (fn: (p: OrdenItem[]) => OrdenItem[]) => {
    if (activeTab === 'viernes') setState(s => ({ ...s, itemsViernes: fn(s.itemsViernes) }));
    else                         setState(s => ({ ...s, itemsDomingo: fn(s.itemsDomingo) }));
  };

  const selViernes = state.programaMes.viernes.find(v => v.id === selectedViernesId) ?? state.programaMes.viernes[0];
  const selDomingo = state.programaMes.domingos.find(d => d.id === selectedDomingoId) ?? state.programaMes.domingos[0];
  ...
  const saveEditItem = () => {
    if (!editingId) return;
    setCurrentItems(prev => prev.map(it => it.id !== editingId ? it : {
      ...it, ...editForm,
  ...
  const saveEquipo = () => {
    ...
    setState(s => ({ ...s, equipo: s.equipo.map(m => m.id === editEquipoId ? { ...m, ...equipoForm } : m) }));
    setEditEquipoId(null); setEquipoForm({ name: '', role: '' });
  };
  const addEquipoMember = (grupo: GrupoEquipo) => {
    if (!newEquipo.name.trim()) return;
    setState(s => ({ ...s, equipo: [...s.equipo, { id: `eq_${Date.now()}`, name: newEquipo.name.trim(), role: newEquipo.role.trim(), grupo }] }));
    setNewEquipo({ name: '', role: '' }); setAddingEquipoGrupo(null);
  };
  const deleteEquipo = (id: string) => setState(s => ({ ...s, equipo: s.equipo.filter(m => m.id !== id) }));

  // ── Novedades ──────────────────────────────────────────────────────────────

  const addNovedad = () => {
    if (!novedadForm.titulo.trim()) return;
    setState(s => ({ ...s, novedades: [{ id: `n_${Date.now()}`, ...novedadForm }, ...s.novedades] }));
    setNovedadForm({ tipo: 'info', titulo: '', texto: '' }); setShowNovedadForm(false);
  };
  const deleteNovedad = (id: string) => setState(s => ({ ...s, novedades: s.novedades.filter(n => n.id !== id) }));
  ...
  const TABS: { id: Tab; label: string; icon: React.ReactNode }[] = [
    { id: 'viernes',   label: 'Viernes',          icon: <Music2 size={13} /> },
    { id: 'domingo',   label: 'Domingo',           icon: <BookOpen size={13} /> },
    { id: 'mes',       label: 'Programa del Mes',  icon: <CalendarDays size={13} /> },
    { id: 'equipo',    label: 'Equipo',            icon: <Users size={13} /> },
    { id: 'novedades', label: 'Novedades',         icon: <Bell size={13} /> },
  ];

  // Generic programa section renderer
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                {fields.map(f => (
                  <th key={f.key} className="px-4 py-2 text-left text-[10px] font-bold text-slate-400 uppercase tracking-wider whitespace-nowrap">{f.label}</th>
                ))}
            ...
            <tbody>
              {data.map((row: any) =>
                editingPrograId === row.id ? (
                  <tr key={row.id} className="bg-blue-50">
                    {fields.map(f => (
                      <td key={f.key} className="px-3 py-1.5">
                        <input
                          className="w-full text-xs border border-slate-200 rounded px-2 py-1 bg-white focus:outline-none focus:border-blue-400 min-w-[90px]"
                          value={editPrograForm[f.key] ?? row[f.key] ?? ''}
                          onChange={e => setEditPrograForm(p => ({ ...p, [f.key]: e.target.value }))}
                        />
                      </td>
                    ))}
                  ...
                  <tr key={row.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors">
                    {fields.map(f => (
                      <td key={f.key} className="px-4 py-2.5 text-slate-700">{row[f.key] || '—'}</td>
                    ))}
                    {canEdit && (
              ...
              {data.length === 0 && (
                <tr><td colSpan={fields.length + 1} className="px-4 py-4 text-xs text-slate-400 text-center">Sin registros</td></tr>
              )}
            </tbody>
          </table>
        </div>
        {isAdding && (
          <div className="px-5 py-4 border-t border-blue-100 bg-blue-50 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              {fields.map(f => (
                <div key={f.key}>
                  ...
                  <input
                    className="mt-0.5 w-full text-sm border border-slate-200 rounded-lg px-2.5 py-1.5 bg-white focus:outline-none focus:border-blue-400"
                    placeholder={f.placeholder}
                    value={newPrograForm[f.key] ?? ''}
                    onChange={e => setNewPrograForm(p => ({ ...p, [f.key]: e.target.value }))}
                  />
                </div>
              ))}
            </div>
            <div className="flex gap-2 pt-1">
              ...
              <button onClick={() => setAddingPrograSection(null)}
                className="text-xs text-slate-600 border border-slate-200 hover:bg-white px-3 py-1.5 rounded-lg transition-colors">
                Cancelar
              </button>
            </div>
          </div>
  ...
  // ── Timeline section (shared by Viernes + Domingo) ─────────────────────────
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        {isServiceTab && (
          <button onClick={exportEW} className="flex items-center gap-2 border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 px-4 py-2 rounded-xl text-sm font-medium transition-all shadow-sm">
      ...
      <div className="flex gap-1 bg-slate-100 rounded-xl p-1">
        {TABS.map(t => (
          <button key={t.id} onClick={() => { setActiveTab(t.id); setExpandedId(null); setEditingId(null); setEditingPrograId(null); setAddingPrograSection(null); }}
            className={`flex-1 flex items-center justify-center gap-1.5 px-2 py-2 rounded-lg text-[12px] font-medium transition-all ${activeTab === t.id ? 'bg-white text-slate-800 shadow-sm border border-slate-200' : 'text-slate-500 hover:text-slate-700'}`}>
            {t.icon}
      ...
      {activeTab === 'viernes' && (
        <div className="space-y-4">
          {/* Week selector */}
          {state.programaMes.viernes.length > 0 && (
            <div className="bg-white border border-slate-200 rounded-xl px-4 py-3 space-y-2">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Semana del servicio — {state.programaMes.mesLabel}</p>
              <div className="flex gap-2 flex-wrap">
                {state.programaMes.viernes.map(v => (
                  <button key={v.id} onClick={() => setSelectedViernesId(v.id)}
                    className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all border ${(selectedViernesId ?? state.programaMes.viernes[0]?.id) === v.id ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-600 border-slate-200 hover:border-slate-400'}`}>
                    Vie {v.fechaLabel}
                  </button>
                ))}
              </div>
          ...
          {/* Status bar filled from program */}
          <div className="flex items-start gap-3 bg-white border border-slate-200 rounded-xl px-4 py-3 flex-wrap">
            ...
            <div className="flex-1 text-sm text-slate-700 space-y-0.5">
              {selViernes ? (
                <>
                  <div><strong>Viernes {selViernes.fechaLabel}</strong> · {state.programaMes.mesLabel}</div>
                  <div className="text-slate-500 text-xs">Ministro: <span className="font-semibold text-slate-700">{selViernes.ministro}</span> · Tema: <span className="font-semibold text-slate-700">{selViernes.tema}</span></div>
                  <div className="text-slate-500 text-xs">Oración / Ofrenda / Despedida: <span className="font-semibold text-slate-700">{selViernes.oracion}</span></div>
                </>
              ) : (
                <span className="text-slate-400">Sin programa cargado — configura el Programa del Mes</span>
              )}
            </div>
          </div>
          {renderTimeline()}
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">Secuencia Easy Worship — Viernes</p>
      ...
      {activeTab === 'domingo' && (
        <div className="space-y-4">
          {state.programaMes.domingos.length > 0 && (
            <div className="bg-white border border-slate-200 rounded-xl px-4 py-3 space-y-2">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Semana del servicio — {state.programaMes.mesLabel}</p>
              <div className="flex gap-2 flex-wrap">
                {state.programaMes.domingos.map(d => (
                  <button key={d.id} onClick={() => setSelectedDomingoId(d.id)}
                    className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all border ${(selectedDomingoId ?? state.programaMes.domingos[0]?.id) === d.id ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-600 border-slate-200 hover:border-slate-400'}`}>
                    Dom {d.fechaLabel}
                  </button>
                ))}
              </div>
          ...
          <div className="flex items-start gap-3 bg-white border border-slate-200 rounded-xl px-4 py-3 flex-wrap">
            ...
            <div className="flex-1 text-sm text-slate-700 space-y-0.5">
              {selDomingo ? (
                <>
                  <div><strong>Domingo {selDomingo.fechaLabel}</strong> · {state.programaMes.mesLabel}</div>
                  <div className="text-slate-500 text-xs">1er servicio 08:00: <span className="font-semibold text-slate-700">{selDomingo.primerServicio}</span> · 2do servicio 10:00: <span className="font-semibold text-slate-700">{selDomingo.segundoServicio}</span></div>
                  <div className="text-slate-500 text-xs">Tema: <span className="font-semibold text-slate-700">{selDomingo.tema}</span></div>
                </>
              ) : (
                <span className="text-slate-400">Sin programa cargado — configura el Programa del Mes</span>
              )}
          ...
          {renderTimeline()}
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">Secuencia Easy Worship — Domingo</p>
            <div className="space-y-0.5">
              {currentItems.map((it, i) => (
                <div key={it.id} className="flex gap-3 items-center py-1 border-b border-slate-100 last:border-0">
                  <span className="text-xs font-mono text-slate-400 w-5 flex-shrink-0 text-right">{i + 1}</span>
          ...
          {(['medios', 'alabanza', 'pastoral'] as GrupoEquipo[]).map(grupo => (
            <div key={grupo} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{GRUPO_LABELS[grupo]}</h3>
                    ...
                    <div key={m.id} className="p-3 border border-slate-200 rounded-xl flex items-start gap-2">
                      ...
                      {canEdit && (
                        <div className="flex gap-1 flex-shrink-0">
                          <button onClick={e => startEditEquipo(m, e)} className="text-slate-300 hover:text-blue-500 p-1 rounded hover:bg-blue-50 transition-colors" title="Editar"><Pencil size={12} /></button>
                          <button onClick={() => deleteEquipo(m.id)} className="text-slate-300 hover:text-red-500 p-1 rounded hover:bg-red-50 transition-colors" title="Eliminar"><Trash2 size={12} /></button>
                        </div>
                      )}
                    </div>
                  )
      ...
      {activeTab === 'novedades' && (
        <div>
          {state.novedades.length === 0 && !showNovedadForm && (
            <div className="text-center py-16 text-slate-400"><Bell size={36} className="mx-auto mb-3 opacity-20" /><p className="text-sm">Sin novedades registradas</p></div>
          )}
          ...
          {canEdit && (
            showNovedadForm ? (
              ...
              <button onClick={() => setShowNovedadForm(true)} className="w-full flex items-center justify-center gap-2 border border-dashed border-slate-300 text-slate-400 hover:text-slate-600 hover:border-slate-400 hover:bg-slate-50 text-sm py-3 rounded-xl transition-all mt-2">
